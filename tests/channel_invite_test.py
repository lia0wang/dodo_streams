...
def test_invalid_channel():
    ...
    with pytest.raises(InputError):
        channel_invite_v2(auth_id1, invalid_channel, auth_id2)
...
def test_invalid_user():
    ...
    with pytest.raises(InputError):
        channel_invite_v2(auth_id1, channel_id, invalid_u_id)

def test_repeated_user():
    '''tests for valid channel_invite_v1 with repeated auth_user_id'''
    clear_v1()
    user1 = auth_register_v1("AgentSmith@hotmail.com", "abcd1234", "Agent", "Smith")
    auth_id1 = user1['auth_user_id']
    channel = channels_create_v1(auth_id1, 'comp1531', True)
    ...
    output = channel_test_id['channels'][0]['channel_id']
    with pytest.raises(InputError):
        channel_invite_v2(auth_id1, channel_id, auth_id1)
